...
def ler_delay():
    ...
    time.sleep(0.01)
    delay_time = conexaoSerial.read()
    texto_delay.setText(f"taxa enviada: {ord(delay_time)} ms")
...
def update():
    ...
    if conexaoSerial.inWaiting() > 1:
        ...
        #delay_val = float( (ord(delay_time)*256.0)*5.0/1023.0 )
...
texto_delay = pg.TextItem(text="", color=(255,255,0), anchor=(1,0))
p1.addItem(texto_delay)
texto_delay.setPos(800,5) # adiciona o texto na posicao (0,0) do grafico1
...
botao3 = QtGui.QPushButton('Intervalo entre medições')
proxy3.setWidget(botao3)
botao3.clicked.connect(ler_delay)

proxy4 = QtGui.QGraphicsProxyWidget()
botao4 = QtGui.QPushButton('Aumentar taxa')
proxy4.setWidget(botao4)
botao4.clicked.connect(aumenta_delay)

proxy5 = QtGui.QGraphicsProxyWidget()
botao5 = QtGui.QPushButton('Diminuir taxa')
